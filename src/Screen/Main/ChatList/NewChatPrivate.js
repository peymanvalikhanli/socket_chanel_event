...
export default class NewChatPrivate extends Component {
    constructor() {
        ...
        echo = new Echo({
          ...
          echo 
          .channel('laravel_database_private-message.'+server_connection.user_token)
    ...
    clickinput() {
        ...
        server_connection.send_message(this.state.user_data.key,coment,'',this.call_back_send_message,this);
    ...
    render() {
        ...
        var data = this.props.navigation.state.params.pname;

        if (this.state.getData == null) {
            this.setState({ user_data: data, getData: true });
            server_connection.ChatHistory(data.key, this.call_back_chat_history, this);
                            ...
                            <View style={MainCss.modalstyle2}>
                                <View style={MainCss.modalstyle3}>
                                    <View style={{ alignItems: 'center', marginLeft: 10 }}>
                                        <TouchableOpacity activeOpacity={0.8} style={[MainCss.modaltouch, { backgroundColor: '#8c0073' }]} onPress={() => this.SingleAudioPicker()}>
                                            <Icon name="headphones" type="MaterialCommunityIcons" style={{ fontSize: 22, color: '#F5F5F5' }} />
                                        </TouchableOpacity>
                                        <Text style={{ color: '#696969' }}>audio</Text>
                                    </View>
                                    <View style={{ alignItems: 'center', marginLeft: 10 }}>
                                        <TouchableOpacity activeOpacity={0.8} style={[MainCss.modaltouch, { backgroundColor: '#cf08f7' }]} onPress={() => this.SingleFilePicker()}>
                                            <Icon name="file-document-outline" type="MaterialCommunityIcons" style={{ fontSize: 22, color: '#F5F5F5' }} />
                                        </TouchableOpacity>
                                        <Text style={{ color: '#696969' }}>Document</Text>
                                    </View>
                                    <View style={{ alignItems: 'center', marginLeft: 10 }}>
                                        <TouchableOpacity activeOpacity={0.8} onPress={() => this.galery()} style={[MainCss.modaltouch, { backgroundColor: '#6e08c7' }]}>
                                            <Icon name="picture" type="AntDesign" style={{ fontSize: 22, color: '#F5F5F5' }} />
                                        </TouchableOpacity>
                                        <Text style={{ color: '#696969' }}>Galery</Text>
                                    </View>
                                    <View style={{ alignItems: 'center', marginLeft: 10, marginTop: 10 }}>
                                        <TouchableOpacity activeOpacity={0.8} onPress={() => this.camera()} style={[MainCss.modaltouch, { backgroundColor: '#a35c29' }]}>
                                            <Icon name="camera" type="Feather" style={{ fontSize: 22, color: '#F5F5F5' }} />
                                        </TouchableOpacity>
